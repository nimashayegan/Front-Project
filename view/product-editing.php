<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="../assets/css/layout/product-editing.css" type="text/css">
    <title>ویرایش محصولات</title>
</head>
<body dir="rtl">
    
    <div class="div-home">

        <div class="div-body">

            <div class="div-head">

                <label class="lb-1">ویراش  کردن محصولات</label>

                <button type="submit" class="btn b1" >ورود به حساب</button>

            </div>

            <hr>

            <div class="div-contnt">

                <form action="" class="">

                    <label class="lb-2"> نام محصول جدید :</label>
                    <br>
                    <input name="productName" type="text" class="in-1">
                    <br>
                    <br>
                    <label class="lb-2">نوعه محصول جدید :</label>
                    <br>
                    <input name="productType" type="text" class="in-1">
                    <br>
                    <br>
                    <label class="lb-2">رنگ محصول جدید :</label>
                    <br>
                    <input name="colorProduct" type="text" class="in-1">
                    <br>
                    <br>
                    <label class="lb-2">ضمانت محصول جدید :</label>
                    <br>
                    <input name="warantyProduct" type="text" class="in-1">
                    <br>
                    <br>
                    <label class="lb-2">تعداد خرید جدید :</label>
                    <br>
                    <input name="countProduct" type="text" class="in-1">
                    <br>
                    <br>
                    <br>
                    <hr>
                    <br>
                    <button type="submit" class="btn b2">ویراش</button>
                    <button type="reset" class="btn b3">انصراف</button>
                </form>

            </div>

        </div>

    </div>

</body>
</html>